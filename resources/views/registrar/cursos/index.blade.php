                    ...
                    <div class="dataTables_length" id="dataTables-example_length">
                        <label>Mostrar
                            <select name="" id="" v-model="resource_url">
                                <option value="cursos/paginas/10" >10</option>
                                <option value="cursos/paginas/25">25</option>
                                <option value="cursos/paginas/50">50</option>
                                <option value="cursos/paginas/100">100</option>
                ...
                <div class="col-sm-12">
                    <table class="table  table-bordered table-hover dataTable no-footer dtr-inline" id="dataTables-example" role="grid">
                        ...
                        <tr v-for="(curso,index) in cursos" :key="index">
                            ...
                            <td>
                                <button type="button" class="btn btn-default btn-circle " v-on:click="editar(curso.id,index)"><i class="fa fa-pencil"></i></button>
                                <button type="button" class="btn btn-default btn-circle " v-if="(!curso.deleted_at)" v-on:click="eliminar(curso.id)"><i class="fa fa-trash"></i></button>
                                <button type="button" class="btn btn-default btn-circle " v-else v-on:click="restaurar(curso.id,index)"><i class="fa fa-backward"></i></button>
                            </td>
                        </tr>
                        </tbody>
                    </table>
                    <v-paginator :options="options" :resource_url="resource_url" @update="val => cursos = val"></v-paginator>
                </div>
            </div>
        </div>
        @include('registrar.cursos.registrar')
    </div>
@endsection
@section('scripts')
    ...
    <script>
        ...
        var vuejs = new Vue({
            ...
            data: {
                ...
                url:"/cursos",
                resource_url: 'cursos/paginas/10',
                options: {
                    remote_data: 'data',
                ...
                guardar:function () {
                    ...
                    if(this.nuevoRegistro.estado==0){
                        this.$http.put(this.url+'/'+this.registroEnEdicion.id, {
                                _token: this.csrf,
                                nombre: this.registroEnEdicion.nombre,
                            }
                        ).then(response=>{
